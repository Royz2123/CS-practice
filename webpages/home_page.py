...
def write_home_page() -> None:
    ...
    st.write(
        "אנחנו מאמינים שעבודה מול **טסטים** זה חלק בלתי-נפרד מתהליך העבודה במדעי המחשב. לכן, כל תרגיל מורכב מ-3 חלקים - קובץ PDF המכיל את הגדרת התרגיל, קובץ JAVA המכיל את התבנית לתרגיל, וקובץ JAVA אשר מכיל רשימה של טסטים אשר יבדקו את נכונות הפתרון שלכם.")
    write_contact_us()
